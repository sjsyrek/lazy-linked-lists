...
export class List {
   ...
   */
  traverse (f) {
    const consf = (x, ys) => f(x).fmap(a => cons.bind(null, a)).ap(ys)
    return isEmpty(this) ? this.pure(emptyList) : this.foldr(consf, this.pure(emptyList))
  }
  /**
   ...
   */
  then (ys) {
    const c = (a, b) => (b, a) // eslint-disable-line
    const id = a => a
